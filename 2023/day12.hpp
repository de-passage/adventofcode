...
int count_possibilities(const C<State, Args1...> &states,
...
//  std::cout << "LOG: Total expectation == " << total_n << std::endl;
  ...
  for (auto current_state : states) {
    ...
    for (auto current_node : current_depth_level) {
      ...
      case State::Broken:
        ...
        next_depth_level.push_back(create_broken_node(current_node));
        }
      ...
      case State::Operational:
        ...
        next_depth_level.push_back(create_operational_node(current_node));
  ...
  return current_depth_level.size();
}

inline int process_input_line(const std::string &line) {
...
condition_list:
  ...
  return count_possibilities(states, conditions);
}
